...
//    }
    ncclUniqueId id;
    if (workerId == 0) ncclGetUniqueId(&id);
    MPI_Bcast((void *)&id, sizeof(id), MPI_BYTE, 0, MPI_COMM_WORLD);
    printf("workerNum:%d, workerId:%d\n", workerNum,  workerId);


    int GID = workerId % 4;
    Graph *g = (Graph *) malloc(sizeof(Graph));
    g -> GID = GID;
    printf("build graph g -> GID:%d\n", g->GID);
    cudaSetDevice(g -> GID);

    ncclCommInitRank(&comm->comm, workerNum, id, workerId);
    ...
    printf("localVertexSize:%d, globalVertexSize:%d\n", localVertexSize, globalVertexSize);
    ...
    cudaMallocHost((void **) &srcId, sizeof(float) * edgeSize);
    cudaMallocHost((void **) &dstId, sizeof(float) * edgeSize);
    index[0] = 0;
    ...
    for (int i = 0; i < edgeSize; i++) {
        ...
        edgeLenHost[index[local_u] + addDiff[local_u]] = 1.0f;
        addDiff[local_u]++;
    }

    cudaMallocManaged((void **) &(g->Global2Local), sizeof(int) * globalVertexSize); // debug
    ...
    cudaMalloc((void **) &(g->srcID), sizeof(int) * edgeSize);
    cudaMalloc((void **) &(g->dstID), sizeof(int) * edgeSize);
    ...
    cudaMemcpy(g->srcID, srcId, sizeof(int) * edgeSize, cudaMemcpyHostToDevice);
    cudaMemcpy(g->dstID, dstId, sizeof(int) * edgeSize, cudaMemcpyHostToDevice);
...
__global__ void setMaster2Workers(int *Master2Workers, int *MasterWorkerIndex, int *masterVertex, int *mirrorNumberSum, int *mirrorWorkers, int *Global2Local, int masterSize) {
    ...
    while (id < masterSize) {
        int localVertexId = Global2Local[masterVertex[id]];
//        if (localVertexId == 513638) {
//            printf("id: %d, localVertexId: %d\n", id, localVertexId);
//        }
//        if (id % 1000 == 0) {
...
//            if (localVertexId == 513638) {
//                printf("Master2Workers[%d]: %d\n", MasterWorkerIndex[localVertexId] + i, Master2Workers[MasterWorkerIndex[localVertexId] + i]);
//            }
        }
...
//    if (g -> GID == 0) sleep(3);
    ...
    printf("addMasterRoute: device: %d, localVertexSize:%d, masterSize:%d\n", g -> GID, localVertexSize, masterSize);
    cudaSetDevice(g -> GID);
    ...
    CHECK(cudaMallocManaged((void **) &g->MasterWorkerIndex, sizeof(int) * (localVertexSize + 1))); // debug
    ...
    for (int i = 1; i <= mirrorWorkerSize; i++) {
    ...
    CHECK(cudaMallocManaged((void **) &g->Master2Workers, sizeof(int) * mirrorWorkerSize)); // debug
//    printf("----- mirrorWorkerSize:%d -----\n", mirrorWorkerSize);
//    printf("masterSize:%d\n", masterSize);
...
void addMirrorRoute(Graph* g, int *mirrorVertex, int *masterWorker, int mirrorSize) {
    ...
    CHECK(cudaMallocHost((void **) &Mirror2WorkerHost, sizeof(int) * localVertexSize));
    CHECK(cudaMallocManaged((void **) &g->Mirror2Worker, sizeof(int) * localVertexSize)); //debug
    for (int i = 0; i < localVertexSize; i++) Mirror2WorkerHost[i] = -1;
    cudaMemcpy(g->Mirror2Worker, Mirror2WorkerHost, sizeof(int) * localVertexSize, cudaMemcpyHostToDevice);
